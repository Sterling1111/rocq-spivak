...
Proof.
  ...
  - intros m c H1. assert (m = 0%nat) as H2 by lia. subst m.
    replace (λ h : R, sum_f 0 0 (λ j, c j * h ^ (j - 0))) with (λ h : R, c 0%nat).
    + apply limit_const.
  ...
  - intros m c H1. assert (m = S n' \/ m <= n')%nat as [H2 | H2] by lia.
    + subst m. replace (λ h : R, sum_f (S n') (S n') (λ j, c j * h ^ (j - S n'))) with (λ h : R, c (S n')).
      * apply limit_const.
      * extensionality h. rewrite sum_f_n_n. replace (S n' - S n')%nat with 0%nat by lia. simpl. lra.
    + apply limit_eq' with (f1 := λ h : R, sum_f m n' (λ j, c j * h ^ (j - m)) + c (S n') * h ^ (S n' - m)).
      ...
      * assert (c m = c m + c (S n') * 0 ^ (n' - m) * 0) as H3 by lra. rewrite H3.
        ...
        -- replace (S n' - m)%nat with (S (n' - m))%nat by lia.
           apply limit_eq' with (f1 := λ h : R, c (S n') * h ^ (n' - m) * h).
           ++ intros h. simpl. lra.
           ++ apply limit_mult.
...
Lemma lemma_9_4 : ∀ (n : ℕ) S S',
  S = (λ x, x ^ n) -> ⟦ der ⟧ S = S' -> ∀ x, S' x = n * x ^ (n - 1).
Proof.
  intros n S S' H1 H2 x. subst S. pose proof (H2 x) as H3. unfold derivative_at in H3.
  apply limit_unique with (f := λ h, ((x + h) ^ n - x ^ n) / h) (a := 0).
  - apply H3.
  - assert (n = 0%nat \/ (n >= 1)%nat) as [H4 | H4] by lia.
    + subst n. apply limit_eq' with (f1 := λ h, 0).
      * intros h. lra.
      * replace (0%nat * x ^ (0 - 1)%nat) with 0 by (simpl; lra). apply limit_const.
    + apply limit_eq with (f1 := λ h, sum_f 1 n (λ j, (choose n j)%nat * x ^ (n - j) * h ^ (j - 1))).
      * exists 1. split. lra. intros h H_neq. pose proof Binomial_Theorem x h n as H5.
        rewrite sum_f_Si in H5; try lia.
        replace ((choose n 0)%nat * x ^ (n - 0)%nat * h ^ 0) with (x ^ n) in H5.
        2 : { replace (n - 0)%nat with n by lia. rewrite n_choose_0. simpl (h ^ 0). rewrite Rmult_1_l. rewrite Rmult_1_r. reflexivity. }
        replace (sum_f 1 n (λ i : nat, ((choose n i)%nat * x ^ (n - i)) * h ^ i)) with (h * sum_f 1 n (λ i : nat, ((choose n i)%nat * x ^ (n - i)) * h ^ (i - 1))) in H5.
        2 : {
        ...
        rewrite H5.
        set (S_sum := sum_f 1 n (λ i : nat, ((choose n i)%nat * x ^ (n - i)) * h ^ (i - 1))).
        ...
        -- replace (h * S_sum + x ^ n - x ^ n) with (h * S_sum) by lra.
           ...
           symmetry. exact Halg.
      * replace (n * x ^ (n - 1)) with ((choose n 1)%nat * x ^ (n - 1)).
        -- apply limit_eq' with (f1 := λ h, sum_f 1 n (λ j : nat, ((choose n j)%nat * x ^ (n - j)) * h ^ (j - 1))).
