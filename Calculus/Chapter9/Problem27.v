...
Proof.
  ...
  replace ((((n - k)!)%nat : ℝ)) with ((n - k) * ((n - S k)!)%nat).
  2 : {
...
Lemma lemma_9_27 : ∀ k,
  (0 <= k <= n)%nat -> ⟦ der ^ k ⟧ (S_n) = (λ x, k! * (n ∁ k) * x ^ (n - k)).
Proof.
  intros k H1. induction k as [| k IH].
  - simpl. rewrite n_choose_0. extensionality x. rewrite Nat.sub_0_r. unfold S_n. lra.
  - exists (λ x : R, k! * n ∁ k * x ^ (n - k)).
    split.
    + apply IH; lia.
    + apply derivative_ext with (f1' := λ x, k! * n ∁ k * ((n - k)%nat * x ^ (n - k - 1))).
