...
Lemma lemma_9_8_a : ∀ f f' g g' c,
  g = (λ x, f (x + c)) -> ⟦ der ⟧ f = f' -> ⟦ der ⟧ g = g' -> ∀ x, g' x = f' (x + c).
Proof.
  ...
  apply limit_unique with (f := λ h, (f (x + c + h) - f (x + c)) / h) (a := 0).
  - apply limit_eq' with (f1 := λ h, (f (x + h + c) - f (x + c)) / h).
    + intros h. replace (x + h + c) with (x + c + h) by lra. reflexivity.
    + apply H5.
  - apply H4.
Qed.

Lemma lemma_9_8_b : ∀ f f' g g' c,
  g = (λ x, f (c * x)) -> ⟦ der ⟧ f = f' -> ⟦ der ⟧ g = g' -> ∀ x, g' x = c * f' (c * x).
Proof.
...
Lemma lemma_9_8_c : ∀ f f' a,
  (∀ x, f (x + a) = f x) -> differentiable f -> ⟦ der ⟧ f = f' -> (∀ x, f' (x + a) = f' x).
Proof.
  ...
  apply limit_unique with (f := λ h, (f (x + a + h) - f (x + a)) / h) (a := 0).
  - apply H5.
  - apply limit_eq' with (f1 := λ h, (f (x + h) - f x) / h).
