...
Lemma ksqr_div_3_k_div_3 : ∀ k,
  Z.divide 3 (k^2) -> Z.divide 3 k.
Proof.
  intros k [a H1]. unfold Z.divide.
  assert (∃ p, k = 3*p \/ k = 3*p+1 \/ k = 3*p+2) as [p H2] by zforms.
  exists p. lia.
Qed.
...
Proof.
  ...
  assert (H4 : ∃ (z1' : ℤ) (z2' : ℤ), ((sqrt 3 = z1' / z2')%R /\ (∀ x, x > 1 -> ~(x | z1') \/ ~(x | z2')))).
  { apply rational_representation with (z1 := z1) (z2 := z2). apply H2. apply H3. apply H1. }
  destruct H4 as [z1' [z2' [H4 H5]]]. assert (H6 : (sqrt 3 * sqrt 3 = (z1' / z2') * (z1' / z2'))%R) by nra.
  rewrite sqrt_sqrt in H6. 2 : { lra. } assert (H7 : (z1' <> 0 /\ z2' <> 0)) by (apply sqrt_rational_neq_0 with (r := 3%R); lra).
  destruct H7 as [H7 H8]. assert (H9 : ((z1' : ℝ) <> 0)%R) by (apply not_0_IZR; auto). assert (H10 : ((z2' : ℝ) <> 0)%R) by (apply not_0_IZR; auto).
  replace (z1' / z2' * (z1' / z2'))%R with ((z1')^2 / (z2')^2)%R in H6.
  2 : { field. auto. } apply Rmult_eq_compat_r with (r := ((z2')^2)%R) in H6.
  replace ((z1' ^ 2 / z2' ^ 2 * z2' ^ 2)%R) with ((z1')^2)%R in H6.
...
Lemma ksqr_div_6_k_div_6 : ∀ k,
  Z.divide 6 (k^2) -> Z.divide 6 k.
Proof.
  intros k [a H1]; unfold Z.divide;
  assert (∃ p, k=6*p\/k=6*p+1\/k=6*p+2\/k=6*p+3\/k=6*p+4\/k=6*p+5) as [p H2] by zforms;
  exists p; lia.
Qed.
...
Proof.
  ...
  assert (H4 : ∃ (z1' : ℤ) (z2' : ℤ), ((sqrt 6 = z1' / z2')%R /\ (∀ x, x > 1 -> ~(x | z1') \/ ~(x | z2')))).
  { apply rational_representation with (z1 := z1) (z2 := z2). apply H2. apply H3. apply H1. }
  destruct H4 as [z1' [z2' [H4 H5]]]. assert (H6 : (sqrt 6 * sqrt 6 = (z1' / z2') * (z1' / z2'))%R) by nra.
...
Proof.
  ...
  assert (H5 : ∃ (z1' : ℤ) (z2' : ℤ), ((Rpower 2 (1/3) = z1' / z2')%R /\ (∀ x, x > 1 -> ~(x | z1') \/ ~(x | z2')))).
  { apply rational_representation with (z1 := z1) (z2 := z2). apply H3. apply H4. apply H1. }
  destruct H5 as [z1' [z2' [H5 H6]]]. assert (H7 : (Rpower 2 (1/3) * Rpower 2 (1/3) * Rpower 2 (1/3) = (z1' / z2') * (z1' / z2') * (z1' / z2'))%R) by nra.
  assert (H8 : z1' <> 0 /\ z2' <> 0).
  ...
  replace (z1' / z2' * (z1' / z2') * (z1' / z2'))%R with ((z1')^3 / (z2')^3)%R in H7 by (field; apply not_0_IZR; tauto).
    apply Rmult_eq_compat_r with (r := ((z2')^3)%R) in H7. replace ((z1' ^ 3 / z2' ^ 3 * z2' ^ 3)%R) with ((z1')^3)%R in H7 by (field; apply not_0_IZR; tauto).
  ...
  destruct H10 as [k H10]. rewrite H10 in H7. assert (H12 : (z2' ^ 3 = 4 * (k^3)%Z)%R).
  ...
  { replace 3 with (Z.of_nat 3) by auto. exists (2 * k^3). apply eq_IZR. rewrite <- pow_IZR. rewrite mult_IZR. 
    rewrite mult_IZR. replace (2 * (2 * (k^3)%Z))%R with (4 * (k^3)%Z)%R by lra. nra. }
...
Proof.
  ...
  assert (z1 <> 0 /\ z2 <> 0) as [H3 H4].
  { assert (z1 = 0 \/ z2 = 0 \/ z1 <> 0 /\ z2 <> 0) as [H3 | [H3 | H3]] by lia; auto; rewrite H3 in H1; try rewrite Rdiv_0_l in H1; try rewrite Rdiv_0_r in H1; lra. }
  assert (H5 : ∃ (z1' : ℤ) (z2' : ℤ), ((Rpower 3 (1/3) = z1' / z2')%R /\ (∀ x, x > 1 -> ~(x | z1') \/ ~(x | z2')))).
  { apply rational_representation with (z1 := z1) (z2 := z2). apply H3. apply H4. apply H1. }
  destruct H5 as [z1' [z2' [H5 H6]]]. assert (H7 : (Rpower 3 (1/3) * Rpower 3 (1/3) * Rpower 3 (1/3) = (z1' / z2') * (z1' / z2') * (z1' / z2'))%R) by nra.
  assert (H8 : z1' <> 0 /\ z2' <> 0).
  { assert (z1' = 0 \/ z2' = 0 \/ z1' <> 0 /\ z2' <> 0) as [H8 | [H8 | H8]] by lia; auto; rewrite H8 in H5; try rewrite Rdiv_0_l in H5; try rewrite Rdiv_0_r in H5; lra. }
  ...
  destruct H10 as [k H10]. rewrite H10 in H7. assert (H11 : (z2' ^ 3 = 9 * (k^3)%Z)%R).
  ...
  assert (Z.divide 3 (z2'^3)) as H12. { replace 3 with (Z.of_nat 3) by auto. exists (3 * k^3). apply eq_IZR. rewrite <- pow_IZR. rewrite mult_IZR. 
    rewrite mult_IZR. replace (((Z.of_nat 3)%Z : ℝ)) with 3%R by auto. nra. }
