...
Lemma lemma_2_5_a : ∀ n r,
  r <> 1 -> ∑ 0 n (λ i, r ^ i) = (1 - r ^ (n+1)) / (1 - r).
Proof.
...
Lemma lemma_2_5_b : ∀ n r,
  r <> 1 -> ∑ 0 n (λ i, r ^ i) = (1 - r ^ (n+1)) / (1 - r).
Proof.
  intros n r H1. set (Sum := sum_f 0 n (λ i, r ^ i)).
  assert (H2 : r * Sum = sum_f 0 n (λ i, r ^ (S i))).
  ...
  - unfold Sum. rewrite H3 in *. rewrite sum_f_Si; try lia. 
    replace (sum_f 1 n (λ i, 0 ^ i)) with (sum_f 1 n (λ i, 0)).
