...
Proof.
  ...
  - unfold choose at 2.
    ...
    -- replace (n - k)%nat with (S (n - k) - 1)%nat by lia. rewrite Rmult_comm with (r2 := ((fact (S (n - k) - 1))%nat : ℝ)).
       rewrite fact_div' with (n := ((fact n)%nat : ℝ)).
       2 : { lia. } 2 : { apply not_0_INR. apply fact_neq_0. }
       replace (S (n - k))%nat with (n - (k - 1))%nat at 2 by lia.
       rewrite Rmult_comm with (r2 := ((fact k)%nat : ℝ)).
       replace ((fact n)%nat * k / ((fact k)%nat * (fact (n - (k - 1)))%nat) + (fact n)%nat * (S (n - k))%nat / ((fact k)%nat * (fact (n - (k - 1)))%nat)) with
       (((fact n)%nat * k + (fact n)%nat * (S (n - k))%nat) / ((fact k)%nat * (fact (n - (k - 1)))%nat)).
       2 : { nra. }
       rewrite <- Rmult_plus_distr_l. rewrite <- plus_INR. replace (k + S (n - k))%nat with (S n)%nat by lia.
       replace ((fact n)%nat * (S n)%nat) with (((fact (S n))%nat : ℝ)). 2 : { rewrite <- mult_INR. simpl. replace (fact n * S n)%nat with (fact n + n * fact n)%nat by lia.
       reflexivity. }
       unfold choose. assert (H8 : (S n <? k = false)%nat). apply nltb_gt. lia. rewrite H8.
       replace (n - (k - 1))%nat with (S n - k)%nat by lia. reflexivity.
Qed.

Lemma lemma_2_3_b : ∀ n k : nat,
  is_natural (n ∁ k).
Proof.
...
Lemma lemma_2_3_d : ∀ a b n,
  (a + b) ^ n = ∑ 0 n (λ i, (n ∁ i) * a ^ (n - i) * b ^ i).
Proof.
  intros a b n. induction n as [| k IH].
    ...
    - replace ((a + b) ^ (S k)) with ((a + b) * (a + b) ^ k) by (simpl; lra).
      ...
      replace (λ i : nat, choose k i * a ^ (k - i) * b ^ i * a) with (λ i : nat, choose k i * a ^ (k - i + 1) * b ^ i).
      ...
      replace (λ i : nat, choose k i * a ^ (k - i) * b ^ i * b) with (λ i : nat, choose k i * a ^ (k - i) * b ^ (i + 1)).
      ...
      replace (sum_f 0 k (λ i : nat, choose k i * a ^ (k - i) * b ^ (i + 1))) with
      (sum_f 1 (k + 1) (λ i : nat, choose k (i-1) * a ^ (k - (i-1)) * b ^ (i))).
      2 : { rewrite sum_f_reindex' with (i := 0%nat) (n := k%nat) (s := 1%nat). simpl.
      set (f := λ i : nat, choose k (i - 1) * a ^ (k - (i - 1)) * b ^ i).
      set (g := λ x : nat, choose k (x - 1) * a ^ (k - (x - 1)) * b ^ (x - 1 + 1)).
      apply sum_f_equiv.
      - lia.
      ...
      -- rewrite sum_f_Si. 2 : { lia. }
        ...
        --- rewrite sum_f_i_Sn_f with (n := (S (k'' + 1))%nat). 2 : { lia. }
            ...
            replace (sum_f 1 k (λ i : nat, choose k i * a ^ (k - i + 1) * b ^ i) + choose k 0 * a ^ (k - 0 + 1) * b ^ 0 +
            (sum_f 1 k (λ i : nat, choose k (i - 1) * a ^ (k - (i - 1)) * b ^ i) + choose k (S k - 1) * a ^ (k - (S k - 1)) * b ^ S k))
            with (sum_f 1 k (λ i : nat, choose k i * a ^ (k - i + 1) * b ^ i) + sum_f 1 k (λ i : nat, choose k (i - 1) * a ^ (k - (i - 1)) * b ^ i) +
            choose k 0 * a ^ (k - 0 + 1) * b ^ 0 + choose k (S k - 1) * a ^ (k - (S k - 1)) * b ^ S k) by lra.
            rewrite sum_f_sum. assert (H2 : sum_f 1 k (λ x : nat, choose k x * a ^ (k - x + 1) * b ^ x + choose k (x - 1) * a ^ (k - (x - 1)) * b ^ x)
            = sum_f 1 k (λ x : nat, choose (S k) x * a ^ (k - x + 1) * b ^ x)).
            ...
            rewrite Rmult_1_l. replace (sum_f 0 k (λ x : nat, choose (S k) x * a ^ (k - x + 1) * b ^ x)) with (sum_f 0 k (λ i : nat, choose (S k) i * a ^ (S k - i) * b ^ i)).
            2 : { apply sum_f_equiv. lia. intros k0 H3. replace (S k - k0)%nat with (k - k0 + 1)%nat by lia. reflexivity. }
            nra.
Qed.

Lemma lemma_2_3_e_i : ∀ n,
  ∑ 0 n (λ j, n ∁ j) = 2 ^ n.
Proof.
...
Lemma lemma_2_3_e_iii : ∀ n,
  (n >= 1)%nat -> ∑ 0 n (λ l, if Nat.odd l then n ∁ l else 0) = 2 ^ (n - 1).
Proof.
  intros n H1. assert (H2 : 2 * sum_f 0 n (λ l, if Nat.odd l then choose n l else 0) = sum_f 0 n (λ j, choose n j) - sum_f 0 n (λ j, (-1) ^ j * choose n j)).
...
Lemma lemma_2_3_e_iv : ∀ n,
  (n >= 1)%nat -> ∑ 0 n (λ l, if Nat.even l then n ∁ l else 0) = 2 ^ (n - 1).
Proof.
  intros n H1. assert (H2 : 2 * sum_f 0 n (λ l, if Nat.even l then choose n l else 0) = sum_f 0 n (λ j, choose n j) + sum_f 0 n (λ j, (-1) ^ j * choose n j)).
