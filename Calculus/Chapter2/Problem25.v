...
Lemma lemma_2_25_a_i : Inductive (λ r, True).
Proof.
  split; auto.
Qed.

Lemma lemma_2_25_a_ii : Inductive (λ r, r > 0).
Proof.
  split; intros; nra.
Qed.

Lemma lemma_2_25_a_iii : Inductive (λ r, r <> 1 / 2 /\ r > 0).
Proof.
  split; intros; nra.
Qed.

Lemma lemma_2_25_a_iv : ~Inductive (λ r, r > 0 /\ r <> 5).
Proof.
  intros [[H1 H2] H3]. specialize (H3 4). nra.
Qed.

Lemma lemma_2_25_a_v : ∀ (A B : R -> Prop),
  Inductive A /\ Inductive B -> Inductive (λ r, A r /\ B r).
Proof.
