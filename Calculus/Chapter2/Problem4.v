...
Lemma lemma_2_4_a : ∀ l m n,
  ∑ 0 l (λ i, n ∁ i * m ∁ (l - i)) = (n + m) ∁ l.
Proof.
  ...
  - induction n as [| n' IHm].
    -- intros l. destruct l.
      ...
      --- rewrite sum_f_i_Sn_f; try lia. rewrite O_choose_n; try lia. rewrite Rmult_0_l. rewrite Rplus_0_r. rewrite O_choose_n; try lia.
          replace (λ i : nat, choose 0 i * choose 0 (S l - i)) with (λ i : nat, 0).
    ...
    -- intros l. destruct l.
      ...
      --- rewrite sum_f_i_Sn_f; try lia. replace (S l - S l)%nat with 0%nat by lia. rewrite n_choose_0.
          replace (sum_f 0 l (λ i : nat, choose (S n') i * choose 0 (S l - i))) with (sum_f 0 l (λ _, 0)).
  ...
  - intros l. replace (n + S m')%nat with (S (n + m')) by lia. assert (l = 0 \/ l >= 1)%nat as [H1 | H1] by lia.
    ...
    -- rewrite lemma_2_3_a; try lia. pose proof IHn as H2. pose proof IHn as H3.
       ...
       replace (sum_f 0 l (λ i : nat, choose n i * choose m' (l - i))) with (choose n l + sum_f 0 (l -1) (λ i : nat, choose n i * choose m' (l - i))).
       ...
       replace (λ x : nat, choose n x * choose m' (l - x) + choose n x * choose m' (l - 1 - x)) with (λ x : nat, choose n x * (choose m' (l-x) + choose m' (l - 1- x))).
       2 : { apply functional_extensionality. intros x. lra. }
       replace (sum_f 0 (l - 1) (λ x : nat, choose n x * (choose m' (l - x) + choose m' (l - 1 - x)))) with (sum_f 0 (l-1) (λ x : nat, choose n x * choose (S m') (l - x))).
       ...
       set (f := λ x : nat, choose n x * choose (S m') (l - x)). replace (choose n l * choose (S m') 0) with (f l).
...
Lemma lemma_2_4_b : ∀ n : nat,
  ∑ 0 n (λ k : nat, (n ∁ k) ^ 2) = (2 * n) ∁ n.
Proof.
