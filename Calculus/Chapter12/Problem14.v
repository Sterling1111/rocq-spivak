...
Lemma lemma_12_14_a :
  ∃ f, differentiable f /\ ∀ x, (f x)^5 + f x + x = 0.
Proof.
  set (g := λ y : R, - y ^ 5 - y).
  assert (H1 : ⟦ der ⟧ (λ y, y ^ 5) = λ y, 5 * y ^ 4) by auto_diff.
  assert (H2 : ⟦ der ⟧ (λ y, - y ^ 5) = λ y, - 5 * y ^ 4) by auto_diff.
  assert (H3 : ⟦ der ⟧ (λ y, y) = λ _, 1) by auto_diff.
  assert (H4 : ⟦ der ⟧ g = λ y, -5 * y ^ 4 - 1) by (unfold g; auto_diff).
  assert (H5 : ∀ y, (λ z, -5 * z ^ 4 - 1) y < 0) by (intros; nra).
  assert (H6 : bijective g).
  {
    ...
    - intros x y H7 H8 H9.
      ...
      + assert (H11 : decreasing_on g [x, y]).
        { apply derivative_on_neg_imp_decreasing_on with (f' := λ z, -5 * z ^ 4 - 1); unfold g; auto_diff. }
  ...
  exists f.
  split.
  - assert (H10 : ⟦ der ⟧ f = λ x, / (-5 * (f x) ^ 4 - 1)).
    {
      apply global_inverse_theorem with (f := g) (f' := λ y, -5 * y ^ 4 - 1); auto.
