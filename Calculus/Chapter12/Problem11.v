...
Lemma lemma_12_11 : ∀ f f' f'' f_inv f_inv' f_inv'' x,
  inverse f f_inv ->
  ⟦ der ⟧ f = f' ->
  ⟦ der ⟧ f' = f'' ->
  ⟦ der ⟧ f_inv = f_inv' ->
...
Proof.
  ...
  assert (H8 : ⟦ der x ⟧ (λ y, f' (f_inv y)) = (λ y, f'' (f_inv y) * f_inv' y)) by auto_diff.
  assert (H9 : ⟦ der x ⟧ (λ y, / (f' (f_inv y))) = (λ y, -1 * (f'' (f_inv y) * f_inv' y) / (f' (f_inv y)) ^ 2)).
  { apply derivative_at_inv; [exact H8 | exact H6]. }
  assert (H10 : ⟦ der x ⟧ f_inv' = (λ y, -1 * (f'' (f_inv y) * f_inv' y) / (f' (f_inv y)) ^ 2)).
