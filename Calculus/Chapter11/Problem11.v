...
Lemma A_derivative : ⟦ der ⟧ A (0, ∞) = (λ r, 4 * π * r - 2 * V / r^2).
Proof.
...
Proof.
  apply derivative_on_imp_differentiable_on with (f' := (λ r, 4 * π * r - 2 * V / r^2)).
  apply A_derivative.
Qed.

Lemma optimal_cylinder_volume : ∀ r h,
  Volume r h = V -> 2 * r = h -> V = 2 * π * r^3.
Proof.
  intros r h H2 H3. unfold Volume in H2. rewrite <- H3 in H2. lra.
Qed.

Lemma lemma_11_11 : ∀ r1 h1 r2 h2,
...
Proof.
  ...
  assert (H8 : A r1 <= A r2).
  {
    apply first_derivative_test_domain_min with (f' := λ r, 4 * π * r - 2 * V / r^2) (D := (0, ∞)); try solve [solve_R].
...
Lemma cylinder_r_neq : ∀ r1 h1 r2 h2,
  Volume r1 h1 = V -> Volume r2 h2 = V ->
...
Proof.
  ...
  apply first_derivative_test_domain_strict_min with (f' := λ r, 4 * π * r - 2 * V / r^2) (D := (0, ∞)); try solve [solve_R].
  - apply A_differentiable.
  - apply A_derivative.
