...
Proof.
  ...
  assert (H10 : f (1/2) < 1/2).
  {
    ...
    pose proof lemma_11_30_a' (λ x, 2 * x^2) f (λ x, 4 * x) f' 0 (1/2) ltac:(auto_diff) H1 ltac:(simpl; lra) H10 as [H12 _].
    solve_R.
  }
  set (g := λ x, 1 - f (1 - x)).
  assert (H11 : ⟦ der ⟧ g = λ x, f' (1 - x)) by (unfold g; auto_diff).

  assert (H12 : ∀ x, x ∈ (0, 1 / 2] -> f' (1 - x) < 4 * x).
  {
    intros x' H12.
    ...
    pose proof derivative_at_unique f' f'' (λ _, (f' 1 - f' (1 - x')) / (1 - (1 - x'))) c (H2 c) H16 as H17.
  ...
  assert (H13 : g (1/2) < 1/2).
  {
    ...
    pose proof lemma_11_30_a' (λ x, 2 * x^2) g (λ x, 4 * x) (λ x, f' (1 - x)) 0 (1/2) ltac:(auto_diff) H11 H14 H13 as [H15 _].
    solve_R.
  }
  unfold g in H13.
