...
Proof.
  ...
  { apply continuous_on_eq with (f1 := λ x, f x - g x); auto. apply continuous_on_minus; auto. }
  ...
  - assert (H8 : h 0 >= 0). { change (h 0) with (f 0 - g 0). pose proof (H2 0 ltac:(solve_R)). solve_R. }
    ...
    pose proof intermediate_value_theorem_zero_le (λ x, - h x) 0 1 ltac:(solve_R) ltac:(apply continuous_on_neg; auto) ltac:(solve_R) as [x [H10 H11]].
