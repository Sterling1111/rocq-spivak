...
FOLDER="Calculus/Chapter18/Problem16"
COQ_FILE="Calculus/Chapter18/Problem16.v"
VO_FILE="Calculus/Chapter18/Problem16.vo"

echo "Generating PNG plots for chapter 18 problem 16..."

if ! command -v gnuplot >/dev/null 2>&1; then
    echo "gnuplot not found; skipping plot generation for chapter 18 problem 16." >&2
    exit 0
fi
...
for f in "$FOLDER"/*.gp; do
    ...
    case $base in
        f2) func="f(x) = e^{x} / x^2" ; func_label="e^{x} / x^2" ;;
        *)  func="$base(x)" ; func_label="$base(x)" ;;
    esac
    
    temp_gp="temp_${base}.txt"
    echo "set terminal pngcairo size 800,800 enhance font 'arial,12'" > "$temp_gp"
    echo "set output '${FOLDER}/${base}.png'" >> "$temp_gp"
    echo "set title '${func}'" >> "$temp_gp"
    echo "set size ratio 1.25" >> "$temp_gp"
