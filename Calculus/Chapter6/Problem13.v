...
Lemma lemma_6_13_a : ∀ f a b,
  a < b -> continuous_on f [a, b] -> ∃ g,
    (∀ x, x ∈ [a, b] -> g x = f x) /\ continuous_on g ℝ.
Proof.
  intros f a b H1 H2.
  set (g := λ x, if Rle_dec x a then f a else if Rle_dec x b then f x else f b).
  exists g. split.
