...
Definition swept_area (c v : vector_function 2) t₀ t :=
  ∫ t₀ t (areal_velocity c v).
Definition central_force l u (c acceleration : vector_function 2) :=
  ∀ t, l < t < u → c t ∥ acceleration t.
Definition kepler_second_law l u (c v : vector_function 2) t₀ :=
  ∃ rate, ∀ t, l < t < u → swept_area c v t₀ t = rate * (t - t₀).
...
Lemma swept_area_derivative l u c v acceleration t₀ t :
  (⟦ der ⟧ c (l, u) = v)%vc → (⟦ der ⟧ v (l, u) = acceleration)%vc →
  l < t₀ < u → l < t < u →
  (⟦ der t ⟧ (swept_area c v t₀) = (areal_velocity c v)).
Proof.
  intros H1 H2 H3 H4. unfold swept_area. eapply FTC1_at with
    (c := (l + Rmin t₀ t) / 2) (d := (Rmax t₀ t + u) / 2).
  - destruct (Rle_dec t₀ t) as [H5 | H5]; [rewrite Rmin_left | rewrite Rmin_right]; lra.
  - destruct (Rle_dec t₀ t) as [H5 | H5]; [rewrite Rmax_right | rewrite Rmax_left]; lra.
  - apply continuous_at_imp_continuous_on. intros x H5.
    assert (H6 : l < x < u).
    { unfold Ensembles.In in H5. cbn in H5.
      destruct (Rle_dec t₀ t) as [H6 | H6]; rewrite ?Rmin_left, ?Rmax_right in H5 by lra;
...
Theorem central_iff_constant_momentum l u c v acceleration t₀ :
  (⟦ der ⟧ c (l, u) = v)%vc → (⟦ der ⟧ v (l, u) = acceleration)%vc →
  (∀ t, l < t < u → c t ≠ ⟨0, 0⟩) → l < t₀ < u →
  (central_force l u c acceleration ↔
   ∀ t, l < t < u → angular_momentum c v t = angular_momentum c v t₀).
Proof.
  ...
  - intros H5.
    ...
    intros t H7. exact (scalar_constant l u _ H6 t t₀ H7 H4).
  - intros H8 t H7. apply (proj1 (det_zero_iff_parallel _ _ (H3 t H7))).
    pose proof (scalar_constant_derivative l u (angular_momentum c v)
      (angular_momentum c v t₀) t H7 H8) as H6.
    exact (derivative_at_unique _ _ _ t
      (angular_momentum_derivative c v acceleration t (H1 t H7) (H2 t H7)) H6).
Qed.

Theorem constant_momentum_area l u c v acceleration t₀ M :
  (⟦ der ⟧ c (l, u) = v)%vc → (⟦ der ⟧ v (l, u) = acceleration)%vc →
  l < t₀ < u →
  (∀ t, l < t < u → angular_momentum c v t = M) →
  ∀ t, l < t < u → swept_area c v t₀ t = M / 2 * (t - t₀).
Proof.
  intros H1 H2 H3 H4 t H5.
  assert (H6 : (∀ τ, l < τ < u → ⟦ der τ ⟧ (λ x, swept_area c v t₀ x - M / 2 * (x - t₀)) = (λ _, 0))).
  { intros x H7.
    pose proof (swept_area_derivative l u c v acceleration t₀ x H1 H2 H3 H7) as H8.
    orbit_diff. unfold areal_velocity. rewrite H4 by exact H7. ring. }
  pose proof (scalar_constant l u _ H6 t t₀ H5 H3) as H9.
...
Theorem theorem_17_1 l u c v acceleration t₀ :
  (⟦ der ⟧ c (l, u) = v)%vc → (⟦ der ⟧ v (l, u) = acceleration)%vc →
  (∀ t, l < t < u → c t ≠ ⟨0, 0⟩) → l < t₀ < u →
  (kepler_second_law l u c v t₀ ↔ central_force l u c acceleration).
Proof.
  intros H1 H2 H3 H4. split.
  - intros [rate H5]. apply (proj2 (central_iff_constant_momentum
      l u c v acceleration t₀ H1 H2 H3 H4)).
    assert (H6 : ∀ t, l < t < u → angular_momentum c v t = 2 * rate).
    { intros t H7.
      assert (H8 : (⟦ der t ⟧ (swept_area c v t₀) = (λ _, rate))).
      { eapply scalar_local_ext with (f := λ x, rate * (x-t₀)); eauto.
        - intros x H9. symmetry. auto.
        - orbit_diff. ring. }
      pose proof (derivative_at_unique _ _ _ t
        (swept_area_derivative l u c v acceleration t₀ t H1 H2 H4 H7) H8) as H10.
      unfold areal_velocity in H10. lra. }
    intros t H7. rewrite (H6 t H7), (H6 t₀ H4). reflexivity.
  - intros H11. exists (angular_momentum c v t₀ / 2).
    eapply constant_momentum_area; eauto.
    apply (proj1 (central_iff_constant_momentum l u c v acceleration t₀
