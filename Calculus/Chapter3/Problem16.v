...
Lemma lemma_3_16_a : ∀ (f : R -> R) l,
  (∀ x y, f(x + y) = f x + f y) -> f (standard_sum l) = standard_sum (map f l).
Proof.
  intros f l.
...
Lemma lemma_3_16_b : ∀ f : R -> R,
  (∀ x y, f (x + y) = f x + f y) ->
  ∃ c : R, ∀ x, rational x -> f x = c * x.
Proof.
  intros f H1.
  exists (f 1).
  intros x H2.
  assert (H3 : ∀ n : nat, f n = f 1 * n).
  {
    induction n as [| k IH].
    - pose proof (H1 0 0) as H3. rewrite Rplus_0_l in H3. solve_R.
    - replace (S k) with (k + 1)%nat by lia. rewrite plus_INR, H1. 
      replace (1%nat : ℝ) with 1 by auto. lra.
  }
  assert (H4 : ∀ x : R, f (- x) = - f x).
  {
    intros y.
  ...
  assert (H5 : ∀ (m : nat) (y : R), f (m * y) = m * f y).
  {
    ...
    - replace (S k) with (k + 1)%nat by lia.
      ...
      replace (1%nat : ℝ) with 1 by auto.
      rewrite Rmult_1_l. lra.
  }
  assert (H6 : ∀ n : nat, (n <> 0)%nat -> f (/ n) = f 1 * / n).
  {
    intros n H6.
    specialize (H5 n (/ n)).
  ...
  assert (H8 : ∀ (z : Z) (y : R), f (z * y) = z * f y).
  { 
    intros z w.
    assert (z = 0 \/ z > 0 \/ z < 0)%Z as [H9 | [H9 | H9]] by lia.
    - subst. repeat rewrite Rmult_0_l. specialize (H1 0 0). rewrite Rplus_0_l in H1. lra.
    - replace (z : ℝ) with (((Z.to_nat z)%nat : ℝ)).
      2 : { rewrite INR_IZR_INZ, Z2Nat.id; solve_R. }
      apply H5.
    - replace (z : ℝ) with (- (- z)%Z) by (rewrite opp_IZR; lra).
      replace (- (- z)%Z * w) with (- ((- z)%Z * w)) by lra.
      rewrite H4.
      replace (((- z)%Z : ℝ)) with (((Z.to_nat (- z))%nat : ℝ)) by (rewrite INR_IZR_INZ, Z2Nat.id; solve_R).
      rewrite H5. lra.
  }
  ...
  - rewrite H9. unfold Rdiv. rewrite H8.
    replace (z2 : ℝ) with (((Z.to_nat z2)%nat : ℝ)).
    2 : { rewrite INR_IZR_INZ, Z2Nat.id; solve_R. }
    rewrite H6; [ lra | lia ].
  - rewrite H9. unfold Rdiv. rewrite H8.
    replace (z2 : ℝ) with (- (Z.to_nat (- z2))%nat).
