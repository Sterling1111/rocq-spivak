...
Lemma lemma_10_28 : ∀ f g,
  (∀ x, f x = x * g x) -> continuous_at g 0 -> ∃ f', ⟦ der 0 ⟧ f = f' /\ f' 0 = g 0.
Proof.
...
Lemma lemma_10_28_a : ∀ f g,
  (∀ x, f x = x * g x) -> continuous_at g 0 -> differentiable_at f 0.
Proof.
  intros f g H1 H2.
  exists (g 0).
  apply limit_eq with (f1 := g); auto.
...
Lemma lemma_10_28_b : ∀ f g,
  (∀ x, f x = x * g x) -> continuous_at g 0 -> ⟦ Der 0 ⟧ f = g 0.
Proof.
